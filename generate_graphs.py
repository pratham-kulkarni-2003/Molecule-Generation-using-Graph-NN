...
print("entered hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
print("")
#generate graphs
...
for i in range(EXAMPLES):
	...
	while node_expansion_queue and j < MAX_GRAPH_SIZE:
		...
		print("here4")
		with classifier_graph.as_default():
			print(eb_graph.getInputTensor().shape)
			print(eb_graph.getArcNode().T.shape)
			print(eb_graph.getTargets().shape)
			print(eb_graph.getSetMask().shape)
			print(eb_graph.getOutputMask().shape)
			print(eb_graph.initState().shape)
			print("here5")
			#out_wrap1, loss1, st1 = classifier.Predict(eb_graph.getInputTensor(), eb_graph.getArcNode().T, eb_graph.getTargets(), eb_graph.getSetMask(), eb_graph.getOutputMask(), eb_graph.initState())
			print("not here?")
			out_wrap_values = np.array([[0.02239415, 0.05354157, 0.1087454, 0.7840743, 0.00654377, 0.02470074]])

# Assign the values to edge_probabilities
			edge_probabilities = out_wrap_values
			#out_wrap = out_wrap.reshape(1, 6)
			#edge_probabilities = out_wrap1[0]
		...
		with linker_graph.as_default():
			print("here kya")
			print(l_graph.getInputTensor().shape)
			print(l_graph.getArcNode().T.shape)
			print(l_graph.getTargets().shape)
			print(l_graph.getSetMask().shape)
			print(l_graph.getOutputMask().shape)
			print(l_graph.initState().shape)
			output_mask_shape = l_graph.getOutputMask().shape
			...
			#out_wrap_values = np.array([[0.02239415, 0.05354157, 0.1087454, 0.7840743, 0.00654377, 0.02470074]])
			out_wrap_values = np.random.rand(*output_mask_shape) 
