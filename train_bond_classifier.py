...
OUTPUT_DIM = 3				#target tensor width
...
HIDDEN_UNITS_STATE = 50     #number of units in the hidden layer of the state network
...
EXAMPLES = 13358			#number of examples in the dataset
SPLITTING_SEED = 199   #seed for drawing examples at random for each set
VALIDATION_SIZE = 358      #number of graphs in the validation set
TEST_SIZE = 100			#number of graphs in the test set
GUMBEL_SOFTMAX_TEMPERATURE_MAX = 5.0
GUMBEL_SOFTMAX_TEMPERATURE_MIN = 1.0
debug_state_filename = "Temp/Debug/Classifier/debug_state.txt"
debug_output_filename = "Temp/Debug/Classifier/debug_output.txt"
output_filename = "Temp/Debug/Classifier/output.txt"
path_model = "Temp/Models/Classifier/Classifier"
path_data = "Data/Graphs/"
path_batches = "Temp/Batches/Classifier/"
path_results = "Temp/Results/Classifier/res_classifier_"+RUN_SUFFIX+".txt"
...
param_string = "Classifier_"+RUN_SUFFIX
...
#build directories
if not os.path.exists("Temp/Debug/Classifier"):
	os.makedirs("Temp/Debug/Classifier")
if not os.path.exists("Temp/Batches/Classifier"):
	os.makedirs("Temp/Batches/Classifier")
if not os.path.exists("Temp/Results/Classifier"):
	os.makedirs("Temp/Results/Classifier")
...
if BUILD_BATCHES:
	...
	for g_id in graph_ids_training:
		...
		#drop all the graphs (but the last) in which "STOP" prediction is expected, as the classifier will not run after "STOP" predictions
		drop_indices = list()
		...
		for j in range(len(graph_sequence)-1):
			...
			#add the arc connecting the new node to the node it originated from
			graph_sequence[j].add_edge(output_node_indices[j],new_index)
			graph_sequence[j].edges[output_node_indices[j],new_index]['info'] = translate_bond_direct['candidate']
			#determine the arc target by looking for the same arc in the next graph
			arc_target = graph_sequence[j+1].edges[output_node_indices[j],new_index]['info']
			#translate the arc targets to one hot vectors
			arc_supervision = np.zeros(3, dtype=int)
			arc_supervision[arc_target-1] = 1
			arc_supervision_list.append(arc_supervision)
...
net = standard_net.StandardNet(input_dim = INPUT_DIM, state_dim = STATE_DIM, output_dim = OUTPUT_DIM, hidden_units_state = HIDDEN_UNITS_STATE, hidden_units_output = HIDDEN_UNITS_OUTPUT, namespace="CLASSIFIER_")
model = GNN.GNN(net, max_it=MAX_ITER, input_dim=INPUT_DIM, output_dim = OUTPUT_DIM, state_dim=STATE_DIM, num_train_batches = TRAINING_BATCHES, optimizer=tf.train.AdamOptimizer, learning_rate=LR, threshold=THRESHOLD, param=param_string, namespace="CLASSIFIER_")
